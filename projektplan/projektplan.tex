...
\begin{itemize}
    ...
    \item Proposed a methodology for selecting input variables for energy consumption prediction using XAI \cite{sim_explainable_2022}. Used Extreme Gradient Boosting (XGBoost), Support Vector Regression (SVR), Light Gradient Boosting Model (LightGBM), and LSTM for prediction. Found that variables with strong impact on the forecast include year, hour, energy consumption difference, temperature, and surface-temperature.
\end{itemize}

\section{Implementation}
%Vad (om något) ska implementeras
The implementation of the project consists of models for predicting electricity consumption, as well as integration of resulting model predictions in the existing database. The models will predict based on electricity consumption data from Umeå Energi's database, idk. Part of the implementation is also to use Explainable AI methods to evaluate the contributions of each variable to the prediction. 
