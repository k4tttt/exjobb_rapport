...
% \usepackage{nomencl}
% \makenomenclature
\usepackage[nohyperlinks]{acronym} % nolist also?
\usepackage{comment}
...
\begin{comment}
\chapter{Some {\LaTeX} tips and tricks}
...
first run latex, then dvips, and finally ps2pdf. Notice the flags
used below, they make sure, the correct fonts are used.
\begin{verbatim}
  > latex thesis.tex
...
So, if you both like to produce PS and PDF documents, you
...
{\LaTeX} does this automagically. First we can tell {\LaTeX} where to
find our pictures,
\begin{verbatim}
    \graphicspath{{pictures/}}
\end{verbatim}
...
picture. This makes {\LaTeX} search for the right one, e.g., a PNG-file if it is a PDF document, otherwise an EPS-file.

You can do all sorts of tricks with the included pictures, where
...
there are two, three or ten. This is all handed and made correct
by BibTeX.
\begin{verbatim}
...
\begin{tabular}{ll}
    \texttt{\{Umeå University\}} & (so it does not become U. University) \\
...
tabularx package) environments. With these environments you can have tables
with a fixed width:
\begin{verbatim}
...
depending on the width of the two last columns.
\begin{verbatim}
...
\input{chapters/appendix.tex}\label{cha:appendix}
